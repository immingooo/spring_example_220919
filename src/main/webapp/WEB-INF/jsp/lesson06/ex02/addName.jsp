	...
	<script>
		$(document).ready(function() {
			...
			$('#nameCheckBtn').on('click', function() {
				//alert('얼럿창 확인');
				// nameStatusArea의 하위태그들 초기화
				$('#nameStatusArea').empty(); // 자식 태그들을 모두 비움
				
				let name = $('#name').val().trim();
				
				// 이름이 입력됐는지 확인 validation 체크
				if (name == '') {
				...
				$.ajax({
					// request
					// 조회목적이여서 get방식으로 보내도 됨. type을 따로 지정안하면 get방식으로 보내짐
					type:"get"
					, url:"/lesson06/ex02/is_duplication"
					, data:{"name":name}
					
					// response(콜백으로 받음)
					, success:function(data) { // data는 무조건 받아와야 함. String이 될 수있고 Object도 될 수 있음(Json으로 오면 알아서 Object로 바꿈)
						//alert(data.is_duplication);
						if (data.is_duplication) {
							$('#nameStatusArea').append('<span class="text-danger">중복된 이름입니다.</span>');
						}
					}
					, error:function(e) { // ajax자체가 실패(request에 작성했던 걸 다시 봐줘야 함)
						alert("실패 " + e);
					}
				});
			});
